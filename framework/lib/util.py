...
def adjust_learning_rate_DP(optimizer, scheduler, epoch):
...
class EarlyStopping:
    def __init__(self, patience=7, verbose=False, delta=0, dim='time_loc', cell_idx=None):
    ...
    def __call__(self, val_loss, model, path):
        ...
        if self.best_score is None:
            ...
            if self.dim == 'time' or self.dim == 'time_feat':
                self.save_checkpoint_time(val_loss, model, path)
    ...
    def save_checkpoint_time(self, val_loss, model, path):
        if self.verbose:
            print(f'Cell:{self.cell_idx} Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
        # torch.save(model.state_dict(), path + '/' + f'checkpoint_cell{self.cell_idx}_epo{self.epo}.pth')
        if self.dim == 'time':
            torch.save(model.state_dict(), path + '/' + f'cell{self.cell_idx}_best_checkpoint.pth')
        elif self.dim == 'time_feat':
            torch.save(model.state_dict(), path + '/' + f'cell{self.cell_idx}_feat_best_checkpoint.pth')
        self.val_loss_min = val_loss
