...
class ParallelEmbeddingLayer(nn.Module):
    def __init__(self, patch_len, model_dim, tcn_channels):
        super(ParallelEmbeddingLayer, self).__init__()
        
        self.full_connected1 = nn.Linear(patch_len, model_dim)
        
        self.full_connected2 = nn.Linear(model_dim, model_dim)

        self.tcn = TemporalConvNet(num_inputs=model_dim, num_channels=tcn_channels, kernel_size=2)

    def forward(self, x):

        main_branch = self.full_connected1(x)  # (batch_size, num_nodes, num_patches, model_dim)
        

        parallel_branch = self.full_connected1(x)  # (batch_size, num_nodes, num_patches, model_dim)
        ...
        parallel_branch = self.full_connected2(parallel_branch)
        parallel_branch = parallel_branch.permute(0, 2, 1)     
        parallel_branch = self.tcn(parallel_branch)          
        parallel_branch = parallel_branch.permute(0, 2, 1)      
