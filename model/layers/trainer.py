...
from lib.util import visual, EarlyStopping, adjust_learning_rate_patchtst, test_params_flop, adjust_learning_rate
import time
import pandas as pd
...
class Runner:
    def __init__(self, iter_time, **configs):
        ...
        patchtst_model = PatchTST(**configs)
        self.device = device
        self.patchtst_model = patchtst_model.to(device) if torch.cuda.is_available() else patchtst_model
        if configs['train']['use_multi_gpu']:
            self.patchtst_model = nn.DataParallel(self.patchtst_model, device_ids=[0, 1])
        self.dim = configs['model']['dim']
        if self.dim == 'time' or self.dim == 'time_feat':
            self.cell_idx = configs['cell_idx']

        total_params = sum(p.numel() for p in self.patchtst_model.parameters() if p.requires_grad)#para num
    ...
    def _select_optimizer(self):
        optimizer = torch.optim.Adam(self.patchtst_model.parameters(), lr=self.configs['train']['lr'])
        return optimizer
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_timestamp, batch_y_timestamp) in enumerate(data_loader):
                ...
                if self.dim == 'time_feat':
                    y_pred = self.patchtst_model(batch_x)[:, :, -1:]
                else:
                    y_pred = self.patchtst_model(batch_x)
    ...
    def train(self):
        ...
        for epoch in range(self.configs['train']['epochs']):
            ...
            for i, (batch_x, batch_y, _, _) in enumerate(train_loader):
                ...
                # encoder - decoder
                if self.dim == 'time_feat':
                    y_pred = self.patchtst_model(batch_x)[:, :, -1:].to(self.device)
                else:
                    y_pred = self.patchtst_model(batch_x)
            ...
            # adjust_learning_rate_patchtst(optimizer, scheduler, epoch + 1)
            # scheduler.step()
            ...
            early_stopping(valid_loss, self.patchtst_model, path)
        ...
        self.patchtst_model.load_state_dict(torch.load(best_model_path))
    ...
    def test(self, load_models=False):
        ...
        if load_models:
            if self.dim == 'time':
                self.patchtst_model.load_state_dict(state_dict=torch.load(os.path.join('./checkpoints/patchtst/' + self.model_parameters, 'Time', f'cell{self.cell_idx}_best_checkpoint.pth')))
            elif self.dim == 'time_loc':
                self.patchtst_model.load_state_dict(state_dict=torch.load(os.path.join('./checkpoints/patchtst/' + self.model_parameters, 'Time_Loc', 'best_checkpoint.pth')))
            if self.dim == 'time_feat':
                self.patchtst_model.load_state_dict(state_dict=torch.load(os.path.join('./checkpoints/patchtst/' + self.model_parameters, 'Time_Feat', f'cell{self.cell_idx}_feat_best_checkpoint.pth')))
        pred_list = []
        label_list = []
    ...
    def predict(self, load_models=False):
        ...
        if load_models:
            ...
            self.patchtst_model.load_state_dict(torch.load(best_model_filename))
